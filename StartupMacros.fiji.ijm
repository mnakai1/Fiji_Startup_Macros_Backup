...
function action(directory, filename) {
	open(directory+filename);
	run("Z Project...", "projection=[Max Intensity]");
	run("Make Composite");
	saveAs("Jpeg", directory+filename);
	close();
}

macro "Composite all .nd2 and save as jpeg [F12]" {
	setBatchMode(true);
	dirname = getDirectory("Choose a directory");
	print("dirname set");
	...
	for (i = 0; i < list.length; i++){
		...
		print(origname);
		print("Action...");
		action(dirname, origname);
		print("action done");
		print(i);}
	setBatchMode(false);
}

macro "Max Intensity [F1]" {
	origname = getInfo("image.filename");
	run("Z Project...", "projection=[Max Intensity]");
	setTool("oval");
}

macro "Clear Outside, 8bit, then split channels/close red [F2]" {
	setBackgroundColor(0, 0, 0);
	run("Clear Outside", "stack");
	run("8-bit");
	run("Split Channels");
	close();
	c2max = "C2-MAX_" + origname;
	c1max = "C1-MAX_" + origname;
	selectWindow(c1max);
}

macro "After threshold, mask, divide, multiply, measure [F3]" {
	run("Convert to Mask");
	run("Divide...", "value=255");
	c2max = "C2-MAX_" + origname;
	c1max = "C1-MAX_" + origname;
	imageCalculator("Multiply create", c2max, c1max);
	setThreshold(1, 255);
...
macro "Close Window [F9]" {
	close();
}

macro "Find Maxima [F5]" {
	run("Find Maxima...");
}

macro "Clear, 8bit, then split channels/close red [F6]" {
	setBackgroundColor(0, 0, 0);
	run("Clear", "stack");
	run("8-bit");
	run("Split Channels");
